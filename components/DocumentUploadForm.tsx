...
type Scan={merchant?:string;invoiceNumber?:string;purchaseDate?:string;product?:string;model?:string;serialNumber?:string;price?:number;vat?:number;warrantyMonths?:number;confidence:number;mode?:string};

export function DocumentUploadForm({properties,action}:{properties:Property[];action:(formData:FormData)=>void|Promise<void>}){
  ...
  return <form className="card" action={action}>
    ...
    <label className="scanDrop"><input name="file" type="file" accept="application/pdf,image/jpeg,image/png,image/webp" required onChange={e=>{const f=e.target.files?.[0];if(f&&f.type!=='application/pdf')analyze(f)}}/><span className="scanIcon">🧾</span><strong>{busy?'جاري قراءة الوثيقة...':'صوّر الفاتورة أو ارفعها'}</strong><small>التحليل الذكي للصور، وPDF يُحفظ في الخزنة</small></label>
    {error&&<div className="scanNotice error">{error}</div>}
    {scan&&<div className="scanResult"><div><span className="badge">{scan.mode==='ai'?'AI Extracted':'وضع تجريبي'}</span><strong>{Math.round((scan.confidence||0)*100)}%</strong></div><p>{scan.mode==='ai'?`${scan.merchant||'وثيقة'}${scan.invoiceNumber?` · #${scan.invoiceNumber}`:''}${scan.price?` · ${scan.price} ر.س`:''}`:'واجهة الاستخراج جاهزة، لكن مفتاح AI غير مفعّل.'}</p></div>}
