...
export function DocumentUploadForm({properties,action}:{properties:Property[];action:(formData:FormData)=>void|Promise<void>}){
  ...
  return <><style>{styles}</style><form className="card docAI" action={action}>
    ...
    {scan&&<div className="scanResult"><div><span className="badge">{scan.mode&&scan.mode!=='mock'?'AI Extracted':'وضع تجريبي'}</span><strong>{Math.round((scan.confidence||0)*100)}%</strong></div><p>{scan.mode&&scan.mode!=='mock'?`${scan.merchant||'وثيقة'}${scan.invoiceNumber?` · #${scan.invoiceNumber}`:''}${scan.price?` · ${scan.price} ر.س`:''}`:'واجهة الاستخراج جاهزة، لكن مفتاح AI غير مفعّل.'}</p></div>}
